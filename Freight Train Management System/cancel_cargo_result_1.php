...
<title>Cancel cargo Result</title>
...
<body>
  <?php 
    ...
    $user_id=$_SESSION["id"];
    $sql=" UPDATE reservation SET cargo_status ='CANCELLED' WHERE reservation.pnr_number='".$pnr_number."' AND reservation.user_id='".$user_id."' ";
          ...
          echo "
                  ...
                  <div class=\"text-center\">
                    ...
                    <div class=\"text-center mt-3\">
                        ...
                        <a href=\"http://localhost/railway/cancel_cargo_1.php\">
        ...
        else
        {
            echo "
                  <div class=\"alert alert-success alert-dismissible\">
                      ...
                      <strong>You have successfully Cancelled Your cargo.  <i class=\"fa fa-smile-o\"></i>.

                  </div>
                ";
            $query3 = mysqli_query($conn,"SELECT * FROM cancel_cargo WHERE cancel_cargo.pnr_number='".$pnr_number."' ") or die(mysql_error());
            while ($row121=mysqli_fetch_array($query3))
            {
                $refund=$row121["cargo_fare"];
                echo "
                  <div class=\"alert alert-success alert-dismissible\">
                      <button type=\"button\" class=\"close\" data-dismiss=\"alert\">&times;
                      </button>
                      <strong>Your refund amont is Rs. ".$row121["cargo_fare"]." <i class=\"far fa-smile\"></i>.
                  </div>
                  <div class=\"text-center\">
                    <br>
                    <div class=\"text-center mt-3\">
    ...
    while ( $stations11[$p1]!=$cdrow11['destination_point'] ) 
    {
        ...
        while($y < $stations_count +1 )
        {
            if($quota=='TATKAL')
            {
                $cdquery21="UPDATE tatkal_Cargo SET available_Cargo=available_Cargo+'".$nos."' where train_number='".$cdrow11['train_number']."' AND class_name='".$cdrow11['class_name']."' AND journey_date='".$cdrow11['journey_date']."' AND start_point='".$stations11[$p1]."' AND destination_point='".$stations11[$y]."'";
            }
            else
            {
                $cdquery21="UPDATE classCargo SET available_Cargo=available_Cargo+'".$nos."' where train_number='".$cdrow11['train_number']."' AND class_name='".$cdrow11['class_name']."' AND journey_date='".$cdrow11['journey_date']."' AND start_point='".$stations11[$p1]."' AND destination_point='".$stations11[$y]."'";
            }
            $cdresult21=mysqli_query($conn,$cdquery21);
            $y=$y+1;
        }
    ...
    $cdquery1="SELECT * FROM cancel_cargo WHERE pnr_number='".$cdrow11['pnr_number']."'";
    ...
    $lname=$row[2];
    $eid=$row[3];
        $subject = "cargo Cancellation on Mini IRCTC";

        $sender = "From: MINI IRCTC <iitguwahati21@gmail.com>\r\n";
        $sender .= "MIME-Version: 1.0\r\n";
        $sender .="Content-Type: text/html; charset=ISO-8859-1\r\n";
        $message = "   <html>
                <body>
                    <h1 style=\"color:green;text-align:center\">cargo Cancellation Details</h1>
                    <p>Hi <b>$fname $lname</b>,<br>
                        You have successfully cancelled cargo on Mini IRCTC.cargo details are given below:<br>
                        <br>
                        ...
                        <b>cargo Class:</b><span style=\"margin-right:2px;\"></span> ".$cdrow11['class_name']."<br>
                        <b> Number Of Cargo:</b><span style=\"margin-right:2px;\"></span> ".$cdrow11['number_of_Cargo']."<br>
                        <b>Total Journey Fare:</b><span style=\"margin-right:2px;\"></span>". $cdrow11['cargo_fare1']."<br>
        ...
        //                     <p style=\"color:red\"><b>cargo Cancellation Charges:</b></p>
        //                     <ol>
        //                         <li>If you cancel cargo within 2 days before the scheduled departure of the train, <b>cancellation charges shall be 50%.</b></li>
        //                         <li>If you cancel cargo within 3 to 10 days before the scheduled departure of the train, <b>cancellation charges shall be 20%.</b></li>
