...
<body >
  <?php 
    ...
    if($days!=0)
    {
      $query = mysqli_query($conn,"SELECT t.train_number,t.train_name,,s1.departure_time,c.destination_point,s2.arrival_time,t.arrival_day,c.class_name,c.cargo_fare1,c.available_seats,c.journey_date FROM train_details as t, train_schedule as s1,train_schedule as s2 where s1.train_number=t.train_number AND s2.train_number=t.train_number AND s1.station_name='".$sp."' AND s2.station_name='".$dp."' AND t.train_number=c.train_number AND c.start_point='".$sp."' AND c.destination_point='".$dp."' AND c.journey_date='".$doj."' ");
    }
    else
    {
        $query = mysqli_query($conn,"SELECT t.train_number,t.train_name,c.start_point,s1.departure_time,c.destination_point,s2.arrival_time,t.arrival_day,c.class_name,c.cargo_fare1,c.available_seats,c.journey_date FROM train_details as t,tatkal_seats as c, train_schedule as s1,train_schedule as s2 where t.start_time>=CURRENT_TIME() AND s1.train_number=t.train_number AND s2.train_number=t.train_number AND s1.station_name='".$sp."' AND s2.station_name='".$dp."' AND t.train_number=c.train_number AND c.start_point='".$sp."' AND c.destination_point='".$dp."' AND c.journey_date='".$doj."' ");
    }
                ...
                <table class=\"table table-striped table-hover text-center table-bordered\" id=\"myTable\">
                  <thead>
                    <tr>
                      ...
                      <th class=\"text-center\">cargo Fare (Per Saet)</th>
