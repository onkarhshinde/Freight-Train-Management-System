...
<body>
		...
		if ($conn->connect_error) 
    {
		...
		$cdquery="SELECT distinct pnr_number,passenger_name,passenger_age,passenger_gender,cargo_coach,seat_number,berth FROM passenger_details WHERE pnr_number='".$_GET["pnr_no"]."'";
    ...
    ";
    $cdquery1="SELECT * FROM cancel_cargo WHERE pnr_number='".$_GET["pnr_no"]."'";
    $cdresult1=mysqli_query($conn,$cdquery1);
    while ($cdrow1=mysqli_fetch_array($cdresult1)) 
    {

      echo "
              <p style=\"text-align:center; font-size:20px; margin-top:-25px;margin-bottom:35px;\"><b>Refund Amount :</b> Rs ".$cdrow1['cargo_fare']."</p>
        ";
      }
	?>
